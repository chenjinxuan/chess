...
func UserInit(user models.UsersModel, extra map[string]interface{}, cConf *config.Config) (int, error) {
	...
	hasLogin, _ := databases.Redis.Login.GetInt(r_key)
	databases.Redis.Login.Set(r_key, "1")
	if hasLogin == 1 {
...
func (i *AndroidUserInit) GetUserInitSnapshot(users models.UsersModel, keyCase map[string]interface{}, userInit *config.UserInit) (res UserInitSnapshot, err error) {
	...
	if errDevice == sql.ErrNoRows && len(logs) == 0 {
		res["is_fresh"] = 1
		err = saveRegisterLog(true, users.Id, models.UserInitStatusFresh, res, users.Channel, users.From, ver, "unique_id", uniqueId, userInit)
	} else {
		res["is_fresh"] = 0
		err = saveRegisterLog(false, users.Id, models.UserInitStatusFresh, res, users.Channel, users.From, ver, "unique_id", uniqueId, userInit)
	}
	return
}
...
func (i *IOSUserInit) GetUserInitSnapshot(users models.UsersModel, keyCase map[string]interface{}, userInit *config.UserInit) (res UserInitSnapshot, err error) {
	...
	if errDevice == sql.ErrNoRows && len(logs) == 0 {
		res["is_fresh"] = 1
		err = saveRegisterLog(true, users.Id, models.UserInitStatusFresh, res, users.Channel, users.From, ver, "idfa", idfa, userInit)
	} else {
		res["is_fresh"] = 0
		err = saveRegisterLog(false, users.Id, models.UserInitStatusFresh, res, users.Channel, users.From, ver, "idfa", idfa, userInit)
	}
	return
}
